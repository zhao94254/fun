...
function drawInitialPlaces() {
    ...
    while (i <= rows) {
        ...
        for (col in places[i]) {
            ...
            tr.append('<td data-row="' + i  + '" data-col="' + col  + '" data-name="' + places[i][col]["name"]  + '" class="places-td" id="pCol' + col + '"><div class="tunnel-div"><div class="tunnel-img-container"></div><div style="background-image: url(\'static/assets/tiles/sky/' + random_sky + '.png\')"class="tunnel-goc-div"></div><div style="background-image: url(\'static/assets/tiles/ground/' + random_ground + '.png\')" class="tunnel-goc-div"></div></div></td>');
        }
        if (i == 0) {
            rowspan = rows + 1
            ...
            for (bee in places["Hive"]["insects"]) {
                td.append('<img data-id="' + bee  + '" class="bee-img" src="static/assets/insects/bee.gif">');
            }
