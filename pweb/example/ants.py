...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        place = self.place
        while bee is None and place is not hive\
                and x <= self.max_range and place.entrance:
            # 向后循环。
            place = place.entrance
            x += 1
            bee = random_or_none(place.bees)
        if place is hive or x > self.max_range:
            return None
        return bee
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        ...
        self.imposter = self.queenPlace
        QueenAnt.queenPlace = True
    ...
    def action(self, colony):
        ...
        while place:
            ant = place.ant
            if ant:
                if ant not in self.double:
                    ant.damage *= 2
                    self.double.append(ant)
                if ant.container:
                    ant = ant.ant
                if ant and ant not in self.double:
                    ant.damage *= 2
                    self.double.append(ant)
    ...
    def reduce_armor(self, amount):
        ...
        """
        if not self.imposter and self.armor <= amount:
            bees_win()
        else:
